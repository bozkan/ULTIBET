...
module.exports = {
	...
	transaction: function(type, event, from, to, amount, wagers, server, match, signatures, timestamp, mempoolFile)
	{
		//signatures = signatures.replace(/\+/g, "%2B")

		var transaction = JSON.stringify(new Transaction(type, event, from, to, amount, wagers, server, match, signatures, timestamp))
